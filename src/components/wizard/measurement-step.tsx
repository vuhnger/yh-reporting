...
import { useWizard, Measurement } from "./wizard-context";
...
export function MeasurementStep() {
  const { state, addMeasurement, updateMeasurement, removeMeasurement, nextStep, prevStep } = useWizard();

  // Helper to determine color based on thresholds
  ...
  const handleValueChange = (id: string, field: keyof Measurement, value: string) => {
    ...
    // Only update if valid number (for numeric fields)
